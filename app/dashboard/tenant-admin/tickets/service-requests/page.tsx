...
export default function TenantAdminServiceRequestsPage() {
  ...
  const fetchTickets = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`${API_URL}/tickets?tenantId=${user.tenantId}&kind=service-request`, { headers: getHeaders(true) })
      const result = await response.json()
      if (result.success && result.data) {
        const sorted = result.data.sort(
          (a: any, b: any) => new Date(b.created || b.createdAt).getTime() - new Date(a.created || a.createdAt).getTime()
        )
        setTickets(sorted.filter((ticket: any) => isStrictKindTicket(ticket, "service-request")))
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">Only service-request tickets are shown here</p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Service Request Tickets</CardTitle>
            <CardDescription>Total: {tickets.length} tickets</CardDescription>
          </CardHeader>
          <CardContent>
            {loading && tickets.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">Loading service requests...</div>
            ) : (
