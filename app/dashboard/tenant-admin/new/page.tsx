...
import { BarChart3, Users, Ticket, Settings, Plus } from "lucide-react"
...
export default function TenantAdminNewTicketPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch customers
        const customersResponse = await fetch(`${API_URL}/customers?tenantId=${user.tenantId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(true)
        setTimeout(() => {
          router.push("/dashboard/tenant-admin/tickets")
        }, 1500)
      } else {
