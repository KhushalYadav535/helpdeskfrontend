...
export default function AgentTroubleshootingPage() {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/tickets?myTickets=true&kind=troubleshooting`, {
      ...
      if (result.success && result.data) {
        ...
        setTickets(assignedTickets.filter((ticket: any) => isStrictKindTicket(ticket, "troubleshooting")))
      }
    } catch (error) {
      console.error("Error fetching troubleshooting tickets:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Troubleshooting tickets assigned to you
        ...
        {loading ? (
          <div className="text-center py-12 text-muted-foreground">Loading troubleshooting tickets...</div>
        ) : tickets.length === 0 ? (
          <div className="text-center py-12">
            <Ticket className="h-16 w-16 mx-auto text-muted-foreground mb-4" />
            <h3 className="text-xl font-medium mb-2">No troubleshooting tickets assigned</h3>
            <p className="text-muted-foreground">Troubleshooting tickets will appear here once assigned to you</p>
          </div>
        ) : (
